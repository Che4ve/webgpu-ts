...
export function rotation(axis: Vec3, angle: number): Mat4 {
  ...
  const s = Math.sin(angle), c = Math.cos(angle), v = 1 - c;
  const m = new Float32Array(16);
  m[0] = x * x * v + c;     m[1] = x * y * v + z * s; m[2] = x * z * v - y * s; m[3] = 0;
  m[4] = y * x * v - z * s; m[5] = y * y * v + c;     m[6] = y * z * v + x * s; m[7] = 0;
  m[8] = z * x * v + y * s; m[9] = z * y * v - x * s; m[10]= z * z * v + c;     m[11]= 0;
