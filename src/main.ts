...
async function main() {
  ...
  if (!device) { alert('Не удалось получить устройство WebGPU'); return; }
  ...
  function frame() {
    ...
    const pos: Vec3 = { x: parseFloat(ui.tx.value), y: parseFloat(ui.ty.value), z: parseFloat(ui.tz.value) };
    ...
    const pass = encoder.beginRenderPass({
      colorAttachments: [{ view, clearValue: { r: 0.1, g: 0.1, b: 0.1, a: 1 }, loadOp: 'clear', storeOp: 'store' }],
